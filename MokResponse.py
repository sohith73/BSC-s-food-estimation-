...
# Measurement Mappings (ingredient, unit) -> grams
...
def convert_to_grams(ingredient, quantity, unit):
    ...
    if key in measurement_mappings:
        ...
        if unit in generic_units:
            ...
            print(f"Warning: No measurement mapping for {ingredient} {unit}")
            return None
...
def estimate_nutrition(dish_name):
    ...
    for item in recipe:
        ...
        if grams is None:
            print(f"Warning: Could not convert {ingredient} {quantity} {unit} to grams.")
            continue
...
if __name__ == "__main__":
    ...
    for dish in test_dishes:
        print(f"\nInput: {dish}")
        result = estimate_nutrition(dish)
        print("Output:")
